...
def ecg_peaks(signal, sfreq, smoothwindow=.1, avgwindow=.75,
    ...
    end_qrs = end_qrs[end_qrs > beg_qrs[0]]    # discard QRS ends that precede first QRS start
    ...
    for beg, end in zip(beg_qrs, end_qrs):
        ...
        len_qrs = end - beg
        if len_qrs < min_len:
            continue    # skip QRS that are too short
...
def ppg_peaks(signal, sfreq, peakwindow=.111, beatwindow=.667, beatoffset=.02,
    ...
    end_waves = end_waves[end_waves > beg_waves[0]]    # discard wave-ends that precede first wave-start
