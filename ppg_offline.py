...
from scipy.signal import welch, medfilt, argrelextrema, find_peaks, \
peak_prominences, hilbert
...
def peaks_signal_dev(signal, sfreq):
    ...
    # first feature is simply amplitude of peaks (attenuate outliers with
    # log10; in order to be able to use log, make all values positive)
    height = np.log10(signal_filt[peaks] + 2 * np.abs(min(signal_filt[peaks])))
    
    # second feature is surrounding slope:
    # for each peak p, get the average slope (absolute value) of the PPG in a
    # window beginning at the peak preceding p, extending to the peak following
    # p
    ...
    # the third feature is prominence, basically how much does the peak stand
    # out compared to its immediate neighbours
    ...
    for f in range(features.shape[1]):
        ...
        # remove outliers from all arrays that are (potentially) used in the
        # following
    ...
    x_d = np.linspace(min(height)-np.std(height), max(height)+np.std(height),
                      1000)
    ...
    # use clustering to identify peaks, initialize means with kmeans
    # clustering, this gives clear separation with margin (no enclosed cases)
    # and then fine-tune with GMM to detect irregularities; i.e. best of both
    # worlds
...
#    if n_components > 1:
#        # now use regularized supervised classification to get a less
#        # overfitted decision boundary in case of more than one class   
#        clf = SVC(C=1)
...
#        labels = clf.predict(features)
    ...
    # selecting the cluster containing the peaks based on height (works only in
    # absence of outliers)
    medheight = []
...
def peaks_signal(signal, sfreq):
    ...
    # the fifth feature is the difference in amplitude of each peak to the
    # standard deviation of the amplitude all peaks
    ...
    fmin, fmax = .8, 2.5
    # squaring is important, gives more accurate results!
    f, powden = welch(signal_filt**2, sfreq, nperseg=N)    
...
#    print expNpeaks
    ...
    # use clustering to identify peaks, initialize means with kmeans clustering
    #, this gives clear separation with margin (no enclosed cases) and then
    # fine-tune with GMM to detect irregularities; i.e. best of both worlds
    ...
    # to medheight as the difference increases
    ...
    if ((min(np.abs(label_counts[1] - expNpeaks)) > 0.4 * expNpeaks) or 
        ...
        # throw out potential false positives by cluster label (and then by
    ...
    # now, for each pair of peaks, check if they're closer than 0.5 * the
    # estimated IBI from each other, if this is the case, first check if both
    # of the peaks are members of the labeled cluster, if this is not the case
    # throw the unlabeled peak out; if both peaks are members of the labeled
    # cluster, throw out the one that has the lowest probability of belonging
    # to the cluster
    
    # select the probabilities associated with each labeled peak
    probability = probability[peak_label_idcs, peak_label]    
    n_peaks = np.size(selected_peaks)
    # estimate this from labeled peaks, since frequency estimate can be wrong
    ibi_est = np.median(np.sort(np.diff(selected_peaks))[n_peaks / 2:])    
    remove_p = []
    ...
    for p in np.arange(1, np.size(selected_peaks)):
        ...
        if peakdiff < .6 * ibi_est:
            ...
            # returns [true, true] if both are labeled peaks etc.
            remove = np.in1d(pair, peaks[peak_label_idcs])    
            
            if np.all(remove):    # if all peaks are labeled
                # select the probabilities associated with these labeled peaks
            
                prob = probability[np.in1d(peaks[peak_label_idcs], pair)]
                
                # if both are equally likely to be labeled as peaks correctly
